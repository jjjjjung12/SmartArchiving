...
$(function(){
	var grid = ArchiveGrid.create({
		url: 'GetUserApproveProcAdminList',
		pageSize: 10,
		columns: [
			...
			{name:'APPROVAL_DATE', hidden:true},
...
function popUp(approvalDivCd, reqId){
		...
		if(approvalDivCd == '01') {
			openArchivePopup('userAuthApplyDetailPop?&reqId=' + reqId, {
				width: popWidth - 750,
				height: popHeight - 420,
				keyboard: false
			});
		} else if(approvalDivCd == '03') {
...
$(document).ready(function() {
	...
	$('#btnSave').click(function (e) {
		...
		obj.group_id 			= sessionGroupId;
		obj.user_nm   			= $("input#F_USER_NM_SET").val();
		obj.apply_user_nm 		= sessionUsernm;
		...
		if($("input#F_APPROVAL_YN_RESULT").val() == "N"){
			alert("반려된건입니다");
			return;
		}
...
function loadInfo(selectedRow, currentRow, currentPage) {
		...
		$('#F_APPROVAL_YN_RESULT').val(selectdRow.APPROVAL_YN);
		...
		if(sessionGroupId=="100" || sessionGroupId=="101"){
