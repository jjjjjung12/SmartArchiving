		...
		success: function(data) {
			if (data.result == 'OK') {
				...
				if (!header_data) {
					...
					maskYn = data.maskYn;
					get_message_data(trx_time, message_id, header_data);
				}
			} else {
				$('#CHK_MESSAGE').attr('style', 'display:block;');
				$("label[for='MESSAGE']").text('전문메타 정보가 존재하지 않습니다.');
...
function loadInfoDetail(rowId, currentRow, currentPage) {				
    ...
    get_message_data(sTRX_DTIME, sMESSAGE_ID, sSP_STD_DATA);
};

//Sub 그리드(영문명 한글명 마스킹여부, 마스킹패턴등등출력)
function get_message_data(trx_time, message_id, header_data)
{
...
//			}
			if (val.length > 3) {
				  val = val.substring(0, 3) + "*".repeat(val.length - 3);
			}
			break;
		//CI
		case '14':
