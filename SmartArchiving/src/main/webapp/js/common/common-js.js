	...
	//20241115 8자리
	function toDatePoint2(sDate) {
		var sDateTemp;
		var sReturnDate = "";
		if(sDate != "" && sDate != null){
			sDateTemp = sDate + "";
			sReturnDate = sDateTemp.substr(0,4) + '.' + sDateTemp.substr(4,2) + '.' + sDateTemp.substr(6,2);	
		}
		return sReturnDate;	
	}
	...
	function getServer(object, sData, onDone){
		...
		$.ajax({
			url: ((typeof ctx !== 'undefined') ? ctx : '') + '/GetServerCombo',
			dataType:"json", 
	...
	function getAllSelectOptions(data, val){
		var valNm;
		data.map(function() {
			if($(this).val() == val){
				valNm = $(this).text();
